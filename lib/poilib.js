...
class PoiCont {
    ...
    getPois(targets, allPoi) {		// 指定したtargetsのpoisを返す allPoi:poiViewがfalseでも値を返す
        let tars = [], lls = [], geos = [], geojson = poiCont.pdata.geojson;
        if (targets.length > 0) {
            geos = geojson.filter((geojson_val, geojson_idx) => {
                let found = false;
                for (let target_idx in poiCont.pdata.targets[geojson_idx]) {
                    let target = poiCont.pdata.targets[geojson_idx][target_idx]
                    let inTarget = targets.includes(target)
                    let poiView = allPoi || target == "activity" ? true : Conf.osm[target].expression.poiView // poiViewフィルタはmakeListで行う
                    if ((inTarget || targets == "-") && poiView) {
        ...
        return { geojson: geos, lnglat: lls, targets: tars };
    }
